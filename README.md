...
次は [実装計画書](./IMPLEMENTATION_PLAN.md) のPhase 5完了後の仕上げとして、報告書再編集フローの安定化から開始する。
...
4. 写真なし施工報告書のPDFダウンロード後レイアウトをAndroid、PC、必要に応じてiOSで確認する
5. 写真付き報告書のFirestore保存内容とDrive参照情報を再確認する
6. 管理者、支店長、現場担当者、無効ユーザーの権限制御を確認する
7. Firestoreセキュリティルールを本番向けに整理する
8. 初回パスワード作成フローを実装する
9. OpenAI APIをサーバー経由で接続する
