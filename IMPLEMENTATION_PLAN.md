...
次に着手するべき作業は、写真なし施工報告書PDFレイアウトの最終調整。
...
- 残りは参照元PDFとの最終レイアウト合わせ、長文摘要欄対策、PDF出力品質、権限制御、セキュリティルール整理

次回再開時の作業順:

1. 参照元PDFとアプリの写真なし施工報告書レイアウトを並べて比較する
2. タイトル、控、報告日、会社印、宛先、責任者欄、挨拶文、施工日時、作業時間、施工場所の位置を最終調整する
3. 施工内容、生息状況、作業内容摘要の外枠・内枠・罫線の太さを調整する
4. 摘要欄が長文の場合に、帳票全体が潰れたり下部が見切れたりしないよう調整する
5. PC確認画面、Android確認画面、PDF出力で同じ崩れがないか確認する
6. 調整後に`npm run build`で確認する

次回すり合わせる内容:

- 参照元PDFへの再現度をどこまで求めるか
- 摘要欄が長い場合の扱いを、文字サイズ縮小、欄高さ調整、2枚目分割のどれにするか
- アプリ内PDF確認画面をスクロールなしの縮小表示に寄せるか、読みやすさ優先でスクロール許容にするか
- 写真付き報告書のレイアウト改善を次に進めるか
- Firestore Security Rules、管理者画面の権限制御、Google Drive不要画像整理、AI添削本番API接続の優先順位
