...
写真なし施工報告書PDFまわりの直近確認は完了扱いとし、次に着手するべき作業は本番運用向けの権限制御とFirestoreセキュリティルール整理。
...
- 残りは権限制御、セキュリティルール整理、初回パスワード作成フロー、OpenAI API接続、iOS Safari追加検証
...
6. 各作業ごとに`npm run build`等で検証し、作業単位でコミットする
